...
fun Route.registerEventRoutes(service: EventService) {
    authenticate("auth-jwt") {
        route("/users/me") {
            ...
            get("/doctor-binding") {
                val data = service.getDoctorBinding(call.requireUserId())
                call.respond(ApiResponse(data = data))
            }

            put("/doctor-binding") {
                val request = call.receive<UpsertDoctorBindingRequest>()
                val data = service.upsertDoctorBinding(call.requireUserId(), request)
                call.respond(ApiResponse(data = data))
            }
        }
    }
