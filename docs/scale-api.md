...
- 保存行为：服务端会追加保存每一次原始作答记录；`session.answers` 仍返回每题最新答案快照用于继续作答与展示。
...
TESS 最新版本共 15 个症状项，每个症状项是一个 `TESS_ITEM` 题目；服务端按一题一条答案存储和导出，不再拆成 30 个内部题目。

- `severityOptionId/severityOptionKey`：严重程度，必答且计分，选项分值 `0~4`
- `relationOptionId/relationOptionKey`：药物关联性，必答但不计入总分，选项分值 `0~3`
...
{
  "answer": {
    "severityOptionId": 123,
    "relationOptionId": 128
  }
}
```

规范化后答案快照会包含 `severityScore` 和 `relationScore`。当严重程度选择 `0 - 无` 时，网页会自动把 `relationOptionId` 保存为 `0 - 无关`；服务端也会拒绝“严重程度为 0 但关联性非 0”的答案。
